...
import { selectpostById, useApprovePostMutation, useDisApprovePostMutation, useGetpostsQuery } from "../../features/posts/postsApiSlice";
...
function ApprovePosts() {
    ...
    if (isSuccess) {
        ...
        const allPosts = ids.length
            ? ids.map(id => <Post key={id} postId={id} approveThePost={approveThePost} disapproveThePost={disapproveThePost} />) 
            : 'No posts yet'
            ...
            <MDBCard className="col-5 mx-5 my-5">
                ...
                <MDBCardFooter className='text-muted'>
                    <div className="d-flex justify-content-center">
                        ...
                        {!post?.approved && 
                        <MDBBtn onClick={e => approveThePost(post.id)}>
                            Approve
                        </MDBBtn> }
                        { post?.approved && 
