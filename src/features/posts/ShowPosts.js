...
export default function ShowPosts() {
  ...
  if (isSuccess) {
    const { ids } = posts;
    const allPosts =
      ids.length && ids.map((id) => <PostAuthLess key={id} postId={id} />);
    content = (
      <div className="col-12 row justify-content-center">{allPosts}</div>
    );
  }
      ...
      return (
        <div className="card col-5 mx-5 my-5">
          ...
          <div className="card-body">
            ...
            <div
              dangerouslySetInnerHTML={createMarkup(post.body.substring(0, 80))}
            ></div>
